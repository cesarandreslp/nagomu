...
export default async function Aportes({
  ...
  return (
    <main>
      ...
      {sinCosto ? (
        <p className="error">
          Esta obra todavia no tiene costo determinado por un estudio. Sin costo no hay
          brecha contra la cual medir un aporte.
        </p>
      ) : (
        <>
          ...
          <form action={registrarAporte}>
            ...
            <label>
              ...
              />
              <span className="discreto">
                Obligatorio si el fondo es un traslado presupuestal. Es el costo de
                oportunidad de esta obra, y merece quedar escrito.
              </span>
            </label>
              ...
              <>
                <label>
                  ...
                  <span className="discreto">
                    ...
                    usuario propio. Queda registrado que el actor y quien digito son
                    distintos.
                  </span>
                </label>
                <label>
      ...
      {obra.aportes.length > 0 ? (
        <>
              ...
              <tbody>
                {obra.aportes.map((a) => (
                  <tr key={a.id}>
                    ...
                    <td>
                      {a.actor.nombre}
                      <div className="discreto">{a.actor.tipo.toLowerCase().replace(/_/g, " ")}</div>
                    </td>
                    <td className="discreto">
                      {a.fondo.sigla ?? a.fondo.nombre}
                      {a.proyectoAplazado ? (
                        <div>Se aplazo: {a.proyectoAplazado}</div>
                      ) : null}
                    </td>
          ...
          <p className="discreto">
            Un aporte no se edita: se registra otro que corrige al anterior, y el original
            sigue visible. Cambiar de comprometido a girado tambien es un registro nuevo.
          </p>
        </>
      ) : null}
    </main>
