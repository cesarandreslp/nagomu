 ...
 *
 * Corte PUBLICO del inventario: cantidades por SECTOR doliente y por afectacion, los
 * puntos en el mapa y el lugar general. NUNCA la direccion (`ubicacion`), el dueño ni
 * ningun dato de persona. Esa garantia no es una convencion de nombres: la consulta usa
 * un `select` explicito de solo campos publicos (`CAMPOS_PUBLICOS`), y una prueba contra
 * base (tests/censo.test.ts) verifica que el resultado no contiene la direccion.
 *
 * El agrupamiento por sector es lo que permite que cada doliente (Agricultura, Educacion,
 * Transporte, Salud...) vea lo suyo en departamento y nacion.
 *
 * No recibe sesion: alimenta la vista sin login. El filtro por territorio decide QUE se
 * cuenta, no que se puede ver — lo publico es publico en cualquier nivel.
 */
...
export type CensoPublico = {
  total: number;
  /** Rollup por doliente: lo que cada ministerio/secretaria ve como suyo. */
...
export async function censoPublico(
  ...
  for (const b of bienes) {
    porSector.set(b.sector, (porSector.get(b.sector) ?? 0) + 1);
  ...
  return {
    total: bienes.length,
    porSector: [...porSector].map(([sector, total]) => ({ sector, total })),
